...
export default function FormItemTransfer({ name, value = [], onChange }: FormItemTransferProps) {
  ...
  const projectItems = Array.isArray(value) ? value : [];
  ...
  const moveRight = () => {
    ...
    onChange({ target: { name, value: newProjectItems } });
    setLeftSelected([]);
  };

  const moveLeft = () => {
    const moving = [...rightSelected];
    const newProjectItems = projectItems.filter(p => !moving.some(rs => String(rs.id) === String(p.id)));
    onChange({ target: { name, value: newProjectItems } });
    setRightSelected([]);
  };
  ...
  const availableItems = items
    .filter(p => !projectItems.some(pp => {
      ...
      return String(pp.fk_item) === pId || String(pp.item_id) === pId || String(pp.id) === pId || String(pp.item?.id) === pId || (pp.description === p.description && String(pp.id) === pId);
    }))
  ...
  if (loading) {
    return <Box p={3} textAlign="center"><Typography>Cargando items...</Typography></Box>;
  }
    ...
    <Box>
      <Grid container spacing={2} alignItems="stretch">
            ...
            <List sx={{ height: 300, overflow: 'auto', p: 0 }}>
              {availableItems.map((item) => (
                ...
                >
                  ...
                  <ListItemText 
                    primary={item.description || item.name || item.item_name || `Item #${item.id}`} 
        ...
        <Grid item xs={12} md={5}>
          <Paper elevation={0} sx={{ border: '1px solid #e0e0e0', borderRadius: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
            <Box p={2} borderBottom="1px solid #e0e0e0" bgcolor="#f9fafb" sx={{ borderTopLeftRadius: 8, borderTopRightRadius: 8 }}>
              <Typography variant="subtitle1" fontWeight="600">Items del Producto</Typography>
            ...
            <List sx={{ height: 350, overflow: 'auto', p: 0 }}>
              {projectItems.map((item) => {
                const fullItem = items.find(i => String(i.id) === String(item.id || item.item_id || item.fk_item)) || item;
                return (
                <ListItem
                  key={item.id}
                  component="div"
                ...
                >
                  ...
                  <ListItemText 
                    primary={fullItem.description || fullItem.name || fullItem.item_name || `Item #${fullItem.id}`} 
                    secondary={`Cantidad: ${item.quantity || 0}`}
                  />
                </ListItem>
              )})}
              {projectItems.length === 0 && (
                <Box p={3} textAlign="center" color="text.secondary">No hay items asignados a este producto</Box>
              )}
            </List>
          </Paper>
        </Grid>
